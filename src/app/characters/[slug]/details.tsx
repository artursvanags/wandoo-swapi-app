      ...
      {i && (
        ...
        <div className="pt-32 pb-12 flex flex-col justify-center">
                ...
                <div className="flex flex-wrap pt-2 pb-8 gap-4">
                    ...
                    <p className="text-muted-foreground sm:text-xl flex items-center"><Icons.Cake className="mr-1 w-4 h-4"/>{i.person.birthYear !== "unknown" ? "Born in " + i.person.birthYear : "Unknown birth year"}</p>
                    {i.person.homeworld && i.person.homeworld.name !== "unknown" && (
                        <InfoDialog title={i.person.homeworld.name} content={Object.entries(i.person.homeworld).filter(([key]) => key !== 'created' && key !== 'edited' && key !== '__typename' && key !== 'id').map(([key, value]) => <p key={key}>{`${key}: ${value}`}</p>)} >
                    ...
                    {i.person.species && i.person.species.name !== "unknown" && (
                        <InfoDialog title={i.person.species.name} content={Object.entries(i.person.species).filter(([key]) => key !== 'created' && key !== 'edited' && key !== '__typename' && key !== 'id').map(([key, value]) => <p key={key}>{`${key}: ${value}`}</p>)} >
                            <p className="text-muted-foreground sm:text-xl flex items-center underline"><Icons.Dog className="mr-1 w-4 h-4"/>{Cap(i.person.species.name)}</p>
                        </InfoDialog>
                    )}
                </div>

                <div className="border p-4 rounded">
                    <h2 className="font-heading text-xl sm:text-xl pb-2 mb-2 flex items-center border-b"><Icons.Info className="mr-2 w-4 h-4"/>Main description</h2>
            ...
            <div className=" space-y-10 pb-12">               
                ...
                {i.person.vehicleConnection.vehicles && i.person.vehicleConnection.vehicles.length > 0 && (
                    <div>
                        ...
                        <ul>
                        {i.person.vehicleConnection.vehicles.map((vehicle, index) => (
                            <li key={index}>
                                <InfoDialog title={vehicle.name} content={Object.entries(vehicle).filter(([key]) => key !== 'created' && key !== 'edited' && key !== '__typename' && key !== 'id').map(([key, value]) => <p key={key}>{`${key}: ${value}`}</p>)} >
                                    <p className="text-muted-foreground sm:text-xl flex items-center underline">- {vehicle.name}</p>
                                </InfoDialog>
                            </li>
                        ))}
                        </ul>
